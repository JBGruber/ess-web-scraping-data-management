...
library(rvest)
html <- read_html(
  "https://en.wikipedia.org/w/index.php?title=Opinion_polling_for_the_next_United_Kingdom_general_election&oldid=1366502041"
)
opinion_table <- html |>
...
opinion_tidy <- opinion_table |>
  slice(-1) |>
  pivot_longer(Lab:Others, names_to = "party", values_to = "result") |>
  mutate(
    result_pct = as.numeric(str_extract(result, "\\d+(?=%)")),
    date_clean = str_extract(`Date(s)conducted`, "\\d{1,2} [A-z]{3}"),
    date = lubridate::dmy(paste(date_clean, 2025))
  ) |>
  filter(
    !is.na(result_pct),
    result_pct < 100,
...
) # I'm using an older version of the site since some just changed it
...
pm_table_clean <- pm_table |>
  mutate(
    pm = str_extract(`Prime ministerOffice(Lifespan)`, ".+\\["),
    pm = str_remove(pm, "\\[")
  )
...
html <- read_html(
  "https://www.thomann.co.uk/beyerdynamic_dt770_pro80_ohm_reviews.htm?page=1&order=&page=2&rating=0"
)
reviews <- html |>
...
# 1. Request & collect raw html
html <- read_html(
  "https://www.cnet.com/tech/services-and-software/facebook-hopes-to-normalize-idea-of-data-scraping-leaks-says-leaked-internal-memo/"
)
...
page1_df <- page |>
  ...
  mutate(
    section_title = ifelse(section_title == "", NA_character_, section_title)
  ) |>
