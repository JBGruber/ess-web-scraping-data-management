...
country_df |>
  mutate(
    # with mutate, we can change one of multiple columns or create new ones
...
```{r}
airports |>
  filter(lat > 40.87521, lat < 40.87523, lon > -74.28137, lon < -74.28135) |>
  pull(faa)
```
...
- My first approach would be to just test if `bind_rows` knows how to solve this automagically:
...
- unfortunatly, the list elements are not easily coerced into data.frames, so we need to write a special function and map over the list elements
- I usually start with the first element of the list and develop code to get this into a data.frame

```{r}
# select first element
got_characters_1 <- got_characters |> pluck(1)
# transform information into a tibble/data.frame
tibble(
  name = got_characters_1$name,
  gender = got_characters_1$gender,
  culture = got_characters_1$culture,
  born = got_characters_1$born
  # you can extend the list, but this is the minimum the exercises asked for
)
```

- Now we can wrap this in a function, map over all list elements with it, and combine into one data.frame
...
- When you look at the `seasons` field, you noticed that this is a nested column

```{r}
got_characters_1$tvSeries
```

- Yet, as long as you only have one nested field, you can use a behaviour of `R`: automatic recycling of lenght 1 elements:
...
tibble(
  ...
  culture = got_characters_1$culture,
  born = got_characters_1$born,
  seasons = got_characters_1$tvSeries
)
```

- This is similar to unnesting: the rows of length 1 are recycled (duplicated) until all columns have the same length
...
got_characters |> 
  map(function(element) {
    tibble(
      ...
      culture = element$culture,
      born = element$born,
      seasons = element$tvSeries
    )
  }) |> 
  bind_rows()
```

- Bonus: it would be slightly differnt if you wanted to include another nested field too
- the only way to create a tibble now is to store the columns as lists
...
tibble(
  ...
  culture = got_characters_1$culture,
  born = got_characters_1$born,
  seasons = list(got_characters_1$tvSeries),
  titles = list(got_characters_1$titles)
)
```

- to get the whole table, you would need to do this for all elements, then unnest the nested columns to get a tidy data.frame of the person-season-title observation
...
got_characters |> 
  map(function(element) {
    tibble(
      ...
      culture = element$culture,
      born = element$born,
      seasons = list(element$tvSeries),
      titles = list(element$titles)
    )
  }) |> 
  bind_rows() |> 
  unnest(seasons) |> 
...
- we did something similar in the bonus above. The code is essentially the same, but I'm not unnesting this time
...
- I tried `bind_rows`, which does not error, but somehow produces a data.frame with 1,705 rows from just 155 list elements!
...
- so we take the same approach as before
...
music_collection |> 
  map(function(element) {
    tibble(
      ...
      basic_information = list(element$basic_information)
    )
  }) |> 
  bind_rows()
```

- Now we still need some information from the `basic_information` column (artist name)
- We can again use the first instance to look for the name

```{r}
basic_information_1 <- music_collection[[1]]$basic_information
basic_information_1 |> 
  lobstr::tree()
```

- So the artist's name lives under `basic_information_1$artists[[1]]$name`, but the `[[1]]` indicates there might be more than 1 artist at a time, which we can get with 

```{r}
purrr::map_chr(basic_information_1$artists, function(x) pluck(x, "name"))
```

- now we can do this for the entire list
...
music_collection |> 
  map(function(element) {
    tibble(
      ...
      rating = element$rating,
      basic_information = list(element$basic_information),
      # once again, the other values are recycled if there is more than 1 artist name
      artist_name = purrr::map_chr(element$basic_information$artists, function(x) pluck(x, "name"))
    )
  }) |> 
  bind_rows()
```
...
music_collection |> 
  map(function(element) {
    tibble(
      ...
      # once again, the other values are recycled if there is more than 1 artist name
      artist_name = purrr::map_chr(element$basic_information$artists, function(x) pluck(x, "name")),
      song_title = element$basic_information$title
    )
  }) |> 
  bind_rows()
```
