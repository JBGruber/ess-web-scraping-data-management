...
- What was your worst experience with data and how it could have been prevented?

::: {.notes}
Think about a data processing issue you experienced, you do not have to share it, but you can if you want.

mine: I was a research assistant in a project and only when we reviewed the final version of the paper (after peer review), we noticed that the total number of cases was wrong. We noticed that we accidentally dropped these cases while joining two tables. Luckily it did not change the conclusion of the paper, but we had to change out almost every number and redo all tables.
:::
...
- head to https://www.menti.com/ and vote with code "4880 1541"
...
- **Regular expressions**: string patterns with non-literal meaning that tells a computer what to look for:
  ```{r}
...
## Regular Expression: your "friend" when working with strings {.smaller}

| **Specifier: What to match**                                                                  | Syntax     | Example      | Matches                                                                      |
| :----------------------------------------| :--------: | :----------: | :--------------------------------------------------------------------------: |
| All characters except for new lines      | `.`        | `d.g`        | <code>**dig**</code>, <code>**d!g**</code>                                   |
| Word characters `(letters, digits,_)`    | `\\w`      | `d\\wg`      | <code>**dig**</code>, <code>**dog**</code>,  <code>**`d2g`** </code>                            |
| Digits `(0 to 9)`                        | `\\d`      | `202\\d`     | <code>**2020**</code>, <code>**2021**</code>                                 |
| Whitespace `(space, tab, newline)`       | `\\s`      |              |                                                                              |
| Newline                                  | `\n`       |              |                                                                              |
| Beginning of the string                  | `^`        | `^go`        | <code>**go**go go</code>                           |
| Ending of the string                     | `$`        | `go$`        | <code>go go**go**</code>                           |
...
| Zero or more (non-greedy)                | `*?`       | `d.*?g`      | <code>**dog**g</code>                              |
| One or more                              | `+`        | `".+"`       | <code>**"blue" jacket, "red"**</code>            |
| One or more (non-greedy)                 | `+?`       | `".+"`       | <code>**"blue"** jacket, **"red"**</code>                                 |
...
| Selection of characters                  | `[…]`      | `d[ieou]g`   | <code>**dig**</code>, <code>**dog**</code>, `dug`                            |
| Range of characters in selection         | `[A-z]`    | `[Hhu]`      | <code>**H**i, **h**ow are yo**u**</code>                                     |
| Everything except selection              | `[^...]`   | `[^Hhu]`     | <code>H**i, **h**ow are yo**u</code>                                         |
| Escape special character                 | `\\`       | `3\\.14`     | <code>**3.14**</code> but not `3,14`                                                        |
...
- [Regular Expressions in R Cheat Sheet](https://github.com/rstudio/cheatsheets/blob/main/regex.pdf)
...
country_df <- tibble(input = data_input) |>
  ...
  mutate(
    country = str_extract(input, "^.+?(?= is)"),
    inhabitants = str_extract(
      input,
      "\\d+? million|\\d+\\.\\d+? million|[0-9,]+?(?= inhabitants)"
    ),
    capital = str_extract(input, "(?<=capital is ).+?(?=\\,)"),
    founded = str_extract(input, "\\d{1,2} [A-z]+ \\d{3,4}")
  ) |>
...
country_df |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Number of countries in Europe over time*",
...
country_df |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Largest countries in Europe*",
...
country_df |>
  rows_update(France_updated, by = "country") |>
  filter(founded < "1300-01-01")
```
...
:::{.incremental}
- `cms_patient_experience` contains patient experience data (see `?cms_patient_experience`)
- What is messy here? The `measure_cd` column holds **six different variables** (the `CAHPS_GRP_*` survey measures), and their values are all crammed into the single `prf_rate` column
- As a result each organisation is spread across 6 rows, so *one observation does not form one row*
:::
...
What is a variable and what is an observation depends on the questions you ask of the data, not on some fixed property of the dataset itself
:::

```{r}
#| echo: false
contacts <- tribble(
  ~person    , ~home_phone , ~work_phone ,
  "Amy Chen" , "555-0101"  , "555-0201"  ,
  "Ben Diaz" , "555-0102"  , "555-0201"  ,
  "Cara Fox" , "555-0103"  , "555-0203"
)
```

:::: {.columns}
::: {.column width="50%"}
### Question A: What are this person's numbers?

The observational unit is one *person* &rarr; wide format (`home_phone`/`work_phone` are two variables):
...
:::
::: {.column width="50%"}
### Question B: Is a number shared by multiple people?

The observational unit is one *person-number* pair &rarr; long format (`phone_number` becomes a variable to group by):

```{r}
contacts |>
  pivot_longer(
    cols = c(home_phone, work_phone),
...
players <- sports_results_df |>
  pivot_wider(
    id_cols = c(name, age, scores_id, scores),
    names_from = details_id,
    values_from = details
  ) |>
...
```{r}
list_search(ess_tweets, function(x) {
  str_detect(
    x,
    "Only one week to go until all our session three courses close!"
  )
})
```
