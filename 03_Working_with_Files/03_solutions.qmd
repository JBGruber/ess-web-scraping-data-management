...
curl::curl_download(
  url = "https://api.worldbank.org/v2/en/indicator/IT.NET.USER.ZS?downloadformat=excel",
...
internet_usage |>
  ...
  gghighlight(
    `Country Name` %in%
      c("United Kingdom", "United States", "Germany", "Qatar"),
    use_group_by = FALSE
  ) +
  labs(
    x = NULL,
...
for (speaker in names(corp_hoc_list)) {
  arrow::write_parquet(
    corp_hoc_list[[speaker]],
    paste0("data/house_of_commons_parquet/", speaker, ".parquet")
  )
}
```
...
```{r}
library(arrow)
corp_hoc_con <- open_dataset(
  sources = "data/house_of_commons_parquet",
  format = "parquet"
)
```
