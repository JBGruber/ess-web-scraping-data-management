...
[![](media/win_hidden.jpg)](https://www.autodesk.com/support/technical/article/caas/sfdcarticles/sfdcarticles/How-to-enable-hidden-file-extensions-in-Windows.html){target="_blank"}
:::
...
- The file itself often gives no hint what encoding it uses :crying_cat_face:
:::
...
<!-- ignore this chunk, it is just for rendering -->
```{r}
#| echo: false
if (!interactive()) {
  readLines("data/files/un-secretaries.txt") |>
    iconv(from = "UTF8", to = "ISO-8859-1") |>
    writeLines("data/files/un-secretaries_win.txt", useBytes = TRUE)
}
```
...
readr::read_csv(
  file = "data/files/un-secretaries_win.txt",
...
```{r}
#| eval: false
plenary_speeches_raw <- rio::import(
  "/home/johannes/Dropbox/Teaching/ess-web-scraping-data-management/03_Working_with_Files/data/files/csv-example.csv"
)
```
...
From the perspective of make, everything in the flowchart after the pre-processing step needs to be rebuilt.
...
```{r}
make_quarto <- function(files, destfiles = NULL, output_ext = ".html", ...) {
  if (is.null(destfiles)) {
    destfiles <- paste0(tools::file_path_sans_ext(files), output_ext)
  }
...
read_dat <- function(x) {
  rio::import(x) |>
    mutate(
      kmdist = str_replace(kmdist, ",", "."),
      midist = str_replace(midist, ",", ".")
    ) |>
    mutate(kmdist = as.numeric(kmdist), midist = as.numeric(midist))
}
...
- in these cases you have four choices
...
4. **Do not load the full data into memory**
:::
::: {.column width="30%" }
![Storage vs. Memory](https://www.thetechedvocate.org/wp-content/uploads/2023/07/memory-vs-storage_6019c498d2ad7-660x400@2x.png)
[](https://www.thetechedvocate.org/storage-vs-memory/){target="_blank"}
:::
...
if (!dir.exists("data/house_of_commons")) {
  ...
  for (party in names(corp_hoc_list)) {
    rio::export(
      corp_hoc_list[[party]],
      paste0("data/house_of_commons/", party, ".csv")
    )
  }
}
...
#| cache-lazy: false
library(arrow)
corp_hoc_con <- open_dataset(sources = "data/house_of_commons", format = "csv")
```
...
- you can get a free Github premium account with a university address
...
![](../01_Introduction/media/github.com_JBGruber_PhD_commits_master.png)
...
![](../01_Introduction/media/github.com_project.png)
...
# read
cats_df <- read_sheet(
  "https://docs.google.com/spreadsheets/d/1XdqfPXrZCTim156m8SteKG_teOgj9AJLcAHOMrV_wfI/edit?usp=sharing"
)
# update
...
write_sheet(
  cats_df_updated,
  ss = "https://docs.google.com/spreadsheets/d/1XdqfPXrZCTim156m8SteKG_teOgj9AJLcAHOMrV_wfI/edit?usp=sharing",
  sheet = "gss_cat_updated"
)
```
