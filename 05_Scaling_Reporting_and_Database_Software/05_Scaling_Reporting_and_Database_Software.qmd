---
title: "Session 5:	Database Software and Scaling your Analyses"
...
Note: if you did not set up your own local PostgreSQL server, you can use mine for now. Just replace `localhost` with `204.168.185.249` and `user = "postgres"` with your name in lowercase (e.g., `user = "johannes"`)
...
dbWriteTable(db, name = "airports", value = airports, overwrite = TRUE)
dbWriteTable(db, name = "flights", value = flights, overwrite = TRUE)
dbWriteTable(db, name = "weather", value = weather, overwrite = TRUE)
dbWriteTable(db, name = "planes", value = planes, overwrite = TRUE)
dbWriteTable(db, name = "airlines", value = airlines, overwrite = TRUE)
```
...
Access by typing `localhost:8080` in your browser

![](media/adminer)

::: {.callout-note}
Adminer is only one of several Web-GUIs. pgAdmin would be another popular alternative with more features.
:::
...
tbl(db, "flights") |>
  ...
  filter(!is.na(arr_delay), !is.na(plane_age)) |>
  collect() |>
  group_by(plane_age) |>
...
We need to log in, which you can do without a user by default (use `204.168.185.249´ instead of `localhost` again for a public server):

```{r connect_amcat}
# remotes::install_github("ccs-amsterdam/amcat4r")
library(amcat4r)
amcat_login("http://localhost/api")
```
...
if (!"houseofcommons" %in% list_indexes()$id) {
  corp_hoc_df <- readRDS(
    "../03_Working_with_Files/data/Corp_HouseOfCommons_V2.rds"
  ) |>
...
:::
::: {.column width="35%"}
<iframe src="https://giphy.com/embed/P8ef3Dkynk0xLx1h1T" width="480" height="480" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/script-kiddie-kitty-P8ef3Dkynk0xLx1h1T"></a></p>
:::
:::: 
...
query_aggregate(
  index = "houseofcommons",
  filters = list(party = c("Con", "Lab"), date = list(gte = "2000-01-01")),
...
```{r}
res <- mark(simple = fun1(), complex = fun2())
```
