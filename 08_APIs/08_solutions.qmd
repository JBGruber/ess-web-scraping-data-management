...
They are mostly straightforward to use:


```{r}
curl_translate(
  "curl -X 'GET' \
  'https://members-api.parliament.uk/api/Posts/GovernmentPosts?departmentId=1' \
  -H 'accept: text/plain'"
)
```
...
```{r}
request("https://members-api.parliament.uk") |>
  req_url_path("/api/Posts/GovernmentPosts") |>
  req_method("GET") |>
  req_url_query(
    departmentId = "1",
  ) |>
...
However, it is unclear what valid `departmentId`s exist. 1 doesn't seem to be one of them. However, just below those two, there is another endpoint we can use:


```{r}
request("https://members-api.parliament.uk") |>
  req_url_path("/api/Posts/Departments/") |>
  # 3 valid values according to the docs 0, 1, 2
  req_url_path_append("0") |>
  req_method("GET") |>
  req_headers(
    accept = "text/plain",
  ) |>
...
We can see that, for example, the *HM Treasury* has the ID *50*:
...
And now we get some useful information back.