...
- an API **method**: `GET`, `POST`, `PUT`, `DELETE`, `PATCH` `QUERY`^[Only since June 2026] (only `GET` and sometimes `POST` are important for us )
...
- **body**: the main response containing the requested data (e.g., as JSON or HTML)
...
![](https://httr2.r-lib.org/logo.png){.absolute right=100 top=100 width=350}
...
req <- request("https://content.guardianapis.com") |> # start the request with the base URL
  ...
  req_url_query(
    # instead the query is added to the URL
    q = "parliament AND debate",
    "show-blocks" = "all"
  ) |>
  req_url_query(
    # in this case, the API key is also added to the query
...
- alternatively, use <https://curlconverter.com/r-httr2/>
...
```{r}
search <- request("https://members-api.parliament.uk/api/Members/Search") |>
  req_method("GET") |>
  req_url_query(
    Name = "Major",
    skip = "0",
  ...
  req_perform() |>
  resp_body_json() # <- I added this line to process the response
```
...
tibble(
  ...
  nameDisplayAs = map_chr(items, function(i) {
    pluck(i, "value", "nameDisplayAs")
  }),
  nameFullTitle = map_chr(items, function(i) {
    pluck(i, "value", "nameFullTitle")
  }),
  nameAddressAs = map_chr(items, function(i) {
    pluck(i, "value", "nameAddressAs")
  }),
  gender = map_chr(items, function(i) pluck(i, "value", "gender")),
  latestParty = map(items, function(i) pluck(i, "value", "latestParty")),
  latestHouseMembership = map(items, function(i) {
    pluck(i, "value", "latestHouseMembership")
  }),
...
search_members <- function(name) {
  ...
  return(tibble(
    ...
    nameListAs = map_chr(items, function(i) {
      safe_pluck(i, "value", "nameListAs")
    }),
    nameDisplayAs = map_chr(items, function(i) {
      safe_pluck(i, "value", "nameDisplayAs")
    }),
    nameFullTitle = map_chr(items, function(i) {
      safe_pluck(i, "value", "nameFullTitle")
    }),
    nameAddressAs = map_chr(items, function(i) {
      safe_pluck(i, "value", "nameAddressAs")
    }),
    gender = map_chr(items, function(i) safe_pluck(i, "value", "gender")),
    latestParty = map(items, function(i) safe_pluck(i, "value", "latestParty")),
    latestHouseMembership = map(items, function(i) {
      safe_pluck(i, "value", "latestHouseMembership")
    })
  ))
}
```

## Next Step: testing our function
...
In `R`, the `roxygen2` package handles parsing documentation for packages
...
#' @param membershipended_membershipendreasonids Members who left the House on or after the date given
...
You could look at this documentation using this little helper function.
Usually inside an R package, you just use `devtools::document()`.

```{r}
render_roxygen <- function(text) {
  # 1. roxygen block -> Rd topic object -> .Rd source
  rd <- roxygen2::roc_proc_text(roxygen2::rd_roclet(), text)[[1]]
  rd_file <- tempfile(fileext = ".Rd")
  writeLines(format(rd), rd_file)
  # 2. render and display in browser
  html <- tempfile(fileext = ".html")
  tools::Rd2HTML(rd_file, out = html)
  browseURL(html)
}
```
...
- It is shown in the other common documentation format called [ReDoc](https://github.com/Redocly/redoc)
...
```{r}
res <- request(
  "https://api.semanticscholar.org/graph/v1/author/search?offset=0&limit=100&query=adam+smith"
) |>
  req_retry(
...
```{r}
find_scholar <- function(name, verbose = TRUE) {
  # make initial request
  res <- request("https://api.semanticscholar.org/graph/v1/author/search") |>
    req_url_query(query = name, limit = 100) |>
    req_retry(
      is_transient = function(resp) resp_status(resp) == 429,
      max_tries = 50,
      backoff = function(tries) tries^2
  ...
  while (!is.null(nxt)) {
    # if there are not more results next is empty
    page <- page + 1
    message("\t...fetching page ", page)
    res <- request("https://api.semanticscholar.org/graph/v1/author/search") |>
      req_url_query(query = name, limit = 100, offset = nxt) |>
...
resp <- request("https://api.semanticscholar.org/graph/v1/author/search") |>
  ...
  req_url_query(
    fields = "name,papers.title,papers.year,papers.fieldsOfStudy,papers.authors",
    limit = 10
  ) |>
...
Let's test it with Carolina (director of the summer school):

```{r}
find_scholar("Carolina Garriga")
```
...
5. Look at https://github.com/public-apis/public-apis and wrap one of the free APIs that looks interesting to you
